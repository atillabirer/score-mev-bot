...
function encodeInitialize({ nonce, openTime }: any): Buffer {
  return encodeData({ initialize: { nonce, openTime } }, 1 + 1 + 8);
}

function encodeInitialize2({
  nonce,
  openTime,
  initPcAmount,
  initCoinAmount,
  ...
  return encodeData(
    { initialize2: { nonce, openTime, initPcAmount, initCoinAmount } },
    1 + 1 + 8 + 8 + 8
  );
}

function encodeMonitorStep({
  planOrderLimit,
  placeOrderLimit,
  cancelOrderLimit,
}: any): Buffer {
  return encodeData(
    { monitorStep: { planOrderLimit, placeOrderLimit, cancelOrderLimit } },
    1 + 2 + 2 + 2
  );
}

function encodeDeposit({ maxCoinAmount, maxPcAmount, baseSide }: any): Buffer {
  return encodeData(
    { deposit: { maxCoinAmount, maxPcAmount, baseSide } },
    1 + 8 + 8 + 8
  );
}
...
function encodeMigrateToOpenBook({}: any): Buffer {
...
function encodeWithdrawPnl({}: any): Buffer {
  return encodeData({ withdrawPnl: {} }, 1);
}

function encodeWithdrawSrm({ amount }: any): Buffer {
  return encodeData({ withdrawSrm: { amount } }, 1 + 8);
}

function encodeSwapBaseIn({ amountIn, minimumAmountOut }: any): Buffer {
  return encodeData({ swapBaseIn: { amountIn, minimumAmountOut } }, 1 + 8 + 8);
}

function encodePreInitialize({ nonce }: any): Buffer {
  return encodeData({ preInitialize: { nonce } }, 1 + 1);
}

function encodeSwapBaseOut({ maxAmountIn, amountOut }: any): Buffer {
  return encodeData({ swapBaseOut: { maxAmountIn, amountOut } }, 1 + 8 + 8);
}

function encodeSimulateInfo({
  param,
  swapBaseInValue,
  swapBaseOutValue,
}: any): Buffer {
  return encodeData(
    { simulateInfo: { param, swapBaseInValue, swapBaseOutValue } },
    1 +
      1 +
      1 +
...
function encodeAdminCancelOrders({ limit }: any): Buffer {
  return encodeData({ adminCancelOrders: { limit } }, 1 + 2);
}

function encodeCreateConfigAccount({}: any): Buffer {
  return encodeData({ createConfigAccount: {} }, 1);
}

function encodeUpdateConfigAccount({ param, owner }: any): Buffer {
...
LAYOUT.addVariant(
  0,
  B.struct([B.u8("nonce"), B.u64("openTime")]),
  "initialize"
);
LAYOUT.addVariant(
  1,
...
LAYOUT.addVariant(
  3,
  B.struct([B.u64("maxCoinAmount"), B.u64("maxPcAmount"), B.u64("baseSide")]),
  "deposit"
);
...
LAYOUT.addVariant(
  ...
  B.struct([
    ...
    B.option(
      B.struct([B.u64("lastOrderNumerator"), B.u64("lastOrderDenominator")]),
      "lastOrderDistance"
    ),
    B.option(
      B.struct([B.u64("needTakePc"), B.u64("needTakeCoin")]),
      "needTakeAmounts"
    ),
  ]),
  "setParams"
...
LAYOUT.addVariant(
  11,
  B.struct([B.u64("maxAmountIn"), B.u64("amountOut")]),
  "swapBaseOut"
);
LAYOUT.addVariant(
  12,
...
LAYOUT.addVariant(
  15,
  B.struct([B.u8("param"), B.publicKey("owner")]),
  "updateConfigAccount"
);
